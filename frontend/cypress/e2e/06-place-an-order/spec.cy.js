import { Login, AddToCart, VerifyShoppingCart, FillInShipping } from './helper'
...
describe('Place Order Function', () => {
    ...
    it('Verify successfull adding products to cart', () => {
        ...
        cy.get('.row').find('a[href="/product/66f39d68d3e4433c7c7fc952"]').first().click();
    ...
    //     cy.get('.row').find('a[href="/product/66f39d68d3e4433c7c7fc952"]').first().click();
    //     cy.get('div.row .col select.form-control').find('option[value="0"]').should('not.exist');
    // });

    // // use order that has zero countinstock
    // it('Verify can not add products to cart when status is not in stock', () => {
    //     Login('test@gmail.com', '123456');
    //     cy.get('.row').find('a[href="/product/66f39d68d3e4433c7c7fc957"]').first().click();
    ...
    //     cy.wait(1000);
    //     cy.get('a[href="/cart"]').click();
    //     cy.get('.col-md-8').find('.list-group-item').eq(0).within(() => {
    ...
    //     cy.get('a[href="/cart"]').click();
    //     cy.get('.col-md-8').find('.list-group-item').eq(0).within(() => {
    //         cy.get('.col-md-2').eq(2).find('.form-control').find('option[value="0"]').should('not.exist');
    //     })
    // });
    ...
    //     cy.wait(1000);
    //     cy.get('a[href="/cart"]').click();
    //     cy.get('.col-md-4').find('.list-group-item').eq(1).within(() => {
    ...
    //     let tax = 0
    //     cy.get('.card').within(() => {
    //         cy.get('.list-group-item').contains('Items').parent().find('.col').eq(1).invoke('text').then((itemsValue) => {
    //             itemPrice = itemsValue.substring(1, itemsValue.length);
    //             expect(itemPrice).to.equal('2449.90');
    //         });

    //         cy.get('.list-group-item').contains('Tax').parent().find('.col').eq(1).invoke('text').then((taxValue) => {
    //             tax = taxValue.substring(1, taxValue.length);
    //             cy.log(tax);
    //         })
    //         cy.get('.list-group-item').contains('Total').parent().find('.col').eq(1).invoke('text').then((totalValue) => {
    //             const totalPrice = totalValue.trim().replace('$', '');
    //             return totalPrice;
    //         }).then((totalPrice) => {
    //             // cy.log(Number(itemPrice));
    //             // cy.log((Number(itemPrice) + Number(tax)).toFixed(2));
    //             expect(totalPrice).to.equal((Number(tax) + Number(itemPrice)).toFixed(2));
    //         });
    //     })
    //     cy.get('.row').find('.col-md-8').eq(0).within(()=>{
    //         cy.get('.list-group-item').eq(0).find('p').invoke('text').then((value)=>{
    //             expect(value).to.equal('Address:123 NVC, TPHCM 01000, VN');
    //         })
    //         const listProduct = ['Sony Playstation 5','Airpods Wireless Bluetooth Headphones'];
    //         const listPrice = ['5 x $399.99 = $1999.95','5 x $89.99 = $449.95']
    //         let i=0;
    //         cy.get('.list-group-item').eq(2).within(()=>{
    //             cy.get('.row').each(($el)=>{
    //                 cy.wrap($el).find('.col').invoke('text').then((value)=>{
    //                     expect(value).to.equal(listProduct[i]);
    //                     i++;
    //                 })
    //                 i=0;
    //                 cy.wrap($el).find('.col-md-4').invoke('text').then((value)=>{
    ...
    it('Verify cannot direct to payment method when missing field shipping', () => {
        ...
        cy.contains('button[type="submit"]', 'Continue').click();
        cy.url().should('eq', 'http://127.0.0.1:3000/shipping');
    });
    ...
    it('Verify no products show if the cart is empty', () => {
        ...
        cy.wait(1000);
        cy.get('a[href="/cart"]').click();
        cy.get('div[role="alert"]').should('exist').and('contain.text', 'Your cart is empty');
        cy.get('.card').within(() => {
            cy.get('.list-group-item').eq(0).invoke('text').then((value) => {
